...
<form method="POST" action="{{ isset($compraProduto) ? route('compras-produtos.update', $compraProduto) : route('compras-produtos.store') }}">
  ...
  <div class="mb-3"><label>ID do produto</label><input type="number" class="form-control" name="produto_id" value="{{ old('produto_id', $compraProduto->produto_id ?? '') }}" required></div>
  <div class="mb-3"><label>Fornecedor</label><input class="form-control" name="fornecedor" value="{{ old('fornecedor', $compraProduto->fornecedor ?? '') }}" required></div>
